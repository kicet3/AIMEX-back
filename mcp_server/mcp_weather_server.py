            ...
            if response.status_code == 200:
                ...
                if data.get("response", {}).get("header", {}).get("resultCode") == "00":
                    ...
                    for date in sorted_dates:
                        ...
                        result_parts.append(
                            f"{date_str}: 기온 {temp}°C, {sky_desc}, {pty_desc}"
                        )
            ...
            if response.status_code == 200:
                ...
                if data.get("response", {}).get("header", {}).get("resultCode") == "00":
                    ...
                    for time_key in sorted_times:
                        ...
                        result_parts.append(
                            f"{time_str}: {temp}°C, {sky_desc}, {pty_desc}"
                        )
...
async def get_weather_warning(location: str = "서울") -> str:
    ...
    try:
        ...
        return (
            f"죄송해요! {location}의 기상특보 정보를 가져오는 중에 오류가 발생했어요."
        )
...
async def get_mid_term_forecast(location: str = "서울") -> str:
    ...
    try:
        ...
        return (
            f"죄송해요! {location}의 중기예보 정보를 가져오는 중에 오류가 발생했어요."
        )
...
async def get_weather_observation(location: str = "서울") -> str:
    ...
    try:
        ...
        return (
            f"죄송해요! {location}의 기상관측 정보를 가져오는 중에 오류가 발생했어요."
        )
